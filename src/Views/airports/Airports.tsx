...
import './airports.css';
interface airports{
  chooseOption:{ airport:{setAirportByCountry:(value:boolean)=>void; 
...
const Airports=({chooseOption,setSearchBar}:airports)=>{
    ...
    airports?.results?.map((item:result)=>{
    ...
    const uniqueSortedCountries = Array.from(
      new Map(
        airports?.results?.map(item => [item.components.country, item])
      ).values()
    ).sort((a, b) =>
      a.components.country.localeCompare(b.components.country)
    );
 ...
 uniqueSortedCountries.map((item) => (
  <div className='airport' key={item.components.country}>
    <button
      ...
      onClick={() => {
        chooseOption.airport.country.setCountry(item.components.country);
